    ...
    def train_reward(self, critic):
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                # print(a.shape)
                # print(sa_t_1.shape)
                # Q1, Q2 = critic.forward_v(obs_action=sa_t_1)
                ...
                # print(Q1.shape)
                # Q1 = torch.from_numpy(Q1).float().to(device)
                # print(Q1.sum(axis=1).shape)
                # assert False
                r_hat1 = self.r_hat_member(sa_t_1, member=member)# + lambda1 * Q1
                r_hat2 = self.r_hat_member(sa_t_2, member=member)# + lambda1 * Q1_2
                r_hat1 = r_hat1.sum(axis=1)
                r_hat2 = r_hat2.sum(axis=1)
                # print(r_hat1.float().to(device).shape)
                # assert False
                r_hat = torch.cat([r_hat1, r_hat2], axis=-1)

                # compute loss
                curr_loss = self.CEloss(r_hat, labels) + lambda1 * Q1.sum(axis=1).mean()# + lambda1 * Q1_2.sum(axis=1).mean()
                loss += curr_loss
